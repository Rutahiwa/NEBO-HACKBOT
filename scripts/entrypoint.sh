...
if [ -f "$SERVER_SSL_KEY" ] && [ -f "$SERVER_SSL_CRT" ]; then
    ...
    openssl req \
        ...
        -subj "/C=US/ST=NY/L=NY/O=NEBO-HACKBOT/OU=Project/CN=NEBO-HACKBOT CA" \
    ...
    openssl req \
        ...
        -subj "/C=US/ST=NY/L=NY/O=NEBO-HACKBOT/OU=Project/CN=localhost" \
        -out ${SERVER_SSL_CSR}

    echo "subjectAltName=DNS:nebo-hackbot.local" > extfile.tmp
